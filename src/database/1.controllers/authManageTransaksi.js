  ...
  Uploadverify : (req, res)=>{
    ...
    RejectDataManageTransactionByID : (req,res)=>{
        ...
        db.query(sql,(err,result)=>{
            try{
                if (err) throw err
                var sqlemail = `select * from history;`
                db.query(sqlemail,(errM,resultM)=>{
                    if (errM) throw errM
                    var username = resultM[id-1].username
                    var  Numinvoice = resultM[id-1].invoice_number
                    ...
                    if (err) throw err
                            var to = result1[0].email
                                var mailOption1 = {
                                    ...
                                    to : to ,
                                    subject: 'Information' ,
                                    html : `<p> Maaf pesanan anda dengan nomor invoice ${Numinvoice} kami tolak, dengan alasan pembayaran belum kami terima, silahkan ulangi pembayaran dan upload bukti pembayaran. 
                                            Terima Kasih.</p>`
                                }
                                if (to){
